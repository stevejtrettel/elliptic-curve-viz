...
| 2026-07-06 | View 3 (S² base picture) built: `HopfTorus.baseAt` = η∘rollUp (Steps 1–2; η-convention locked by test against η computed from S³ directly), `sphereToR3` fixes (θ,φ) ↦ ℝ³; `BaseSphere` renderable (unit sphere, profile-curve tube, fiber marks, optional area cap = pole-to-curve fill of area A); `CurveScene.sphere` staged like the plaque, `?sphere=1`; demos/base-sphere pairs per-fiber palette colors on torus and base (E-point marks on the curve deliberately omitted — ST) |
